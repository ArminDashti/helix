...
def _graph_node_allowed(node_id: str) -> bool:
    ...
    if is_sub_agent(definition_id):
        return False
    return definition_id in AGENT_BY_ID
...
def normalize_pipeline_graph(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for entry in raw_nodes:
        ...
        definition_id = resolve_agent_definition_id(node_id)
        if is_sub_agent(definition_id):
            raise ValueError(f"Sub-agent cannot be a pipeline node: {node_id}")
...
def _normalize_stages(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed = known_agent_ids()
    for agent_id in ids:
        if is_sub_agent(agent_id):
            raise ValueError(f"Sub-agent cannot be a pipeline stage: {agent_id}")
...
def get_pipeline_graph_for_mode(mode: str) -> dict[str, Any]:
    """Return the compiled graph for a run mode (research uses a shorter pipeline)."""
    if str(mode or "").strip().lower() == "research":
        return compile_pipeline_flow(research_pipeline_flow())
