...
def logs_collection(request: HttpRequest) -> JsonResponse:
    ...
    kind = str(body.get("kind") or "api").strip()
    duration_s = body.get("duration_s")
    ...
    item = _record_log_error(
        ...
        chart_type=str(body.get("chart_type") or ""),
    )
    return JsonResponse({"log": item}, status=201)
...
def runs_stream(request: HttpRequest) -> HttpResponse:
    ...
    def _stream_with_disconnect_logging() -> Iterator[str]:
        finished = False
        try:
            for chunk in pipeline_events(
                ...
                if '"event": "result"' in chunk or '"event":"result"' in chunk:
                    finished = True
                if '"event": "error"' in chunk or '"event":"error"' in chunk:
                    finished = True
                yield chunk
            ...
            if not finished:
                _record_log_error(
                    kind="api",
                    message="Client disconnected before the run stream finished",
                    prompt=prompt,
                    mode=mode,
                    language=language,
                    path="/api/runs/stream",
