...
12. On reject or warehouse error, return `fail`/`failed` with concrete fixes. Rewrite until success or retry cap (5).
13. Always include `goals` and `what_was_done` in JSON before the validator runs.
